...
{
	my $unit = shift;
	my $th = Net::Amazon::TreeHash->new(unit => $unit);
	my $s = '';
	my $original = join(',', @_);
	for (@_) {
		$s .= $_;
	...
	my $th2 = Net::Amazon::TreeHash->new(unit => $unit);
	$th2->eat_data($s);
	$th2->calc_tree();
...

{
	...
	my $th = Net::Amazon::TreeHash->new(unit => 7);
	$th->eat_data(\$s);
	$th->calc_tree();
	my $th2 = Net::Amazon::TreeHash->new(unit => 7);
	$th2->eat_data($s);
	$th2->calc_tree();
...

{
	...
	my $th = Net::Amazon::TreeHash->new(unit => 1048576);
	$th->eat_data(\$s);
	$th->calc_tree();
	my $hash = $th->get_final_hash();
...

{
	...
	my $th = Net::Amazon::TreeHash->new(unit => $chunk);
	$th->_eat_data_one_mb(\$s);
	$th->calc_tree();
	
	my $th2 = Net::Amazon::TreeHash->new(unit => $chunk);
...

{
	...
	my $th_simple = Net::Amazon::TreeHash->new(unit => $chunk);
	...
	my $th_complex = Net::Amazon::TreeHash->new(unit => $chunk);
...

{
	...
	my $th_complex = Net::Amazon::TreeHash->new(unit => $chunk);
	for (@data) {
		my $th_chunk = Net::Amazon::TreeHash->new(unit => $chunk);
